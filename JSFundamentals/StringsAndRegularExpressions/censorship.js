function censorship(text,censorArr) {
    for(let censorCurr of censorArr){
        let wordDashes='-'.repeat(censorCurr.length);
        let indexOfCensor=text.indexOf(censorCurr);
        while(indexOfCensor>-1) {
            text = text.replace(censorCurr, wordDashes);
            indexOfCensor=text.indexOf(censorCurr);
        }
    }
    console.log(text);
